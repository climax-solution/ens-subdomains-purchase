        ...
        <article className="mt-4">
            <div className="flex items-center mb-4 space-x-4">
                ...
                <div className="space-y-1 font-medium dark:text-white">
                    <p className="text-white flex flex-col gap-2"><span>{data.from}</span> {isLoading ? <time className="animate-pulse w-48 h-4 bg-white rounded-md"/> : <time className="block text-sm text-sky-500 dark:text-sky-400">Joined on {moment(joinedDate).format('LLL')}</time>}</p>
                </div>
            </div>
            <Star
